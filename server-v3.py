...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        print(f"Received POST request with headers: {self.headers}")

        # Extract values from headers
        lcd = int(self.headers.get('LCD', 0))
        sw0 = int(self.headers.get('SW0', 0))
        sw1 = int(self.headers.get('SW1', 0))
        fan = int(self.headers.get('FAN', 0))
        led_red = int(self.headers.get('LEDRED', 0))
        led_green = int(self.headers.get('LEDGRN', 0))
        speed = int(self.headers.get('SPEED', 0))
        slider0 = int(self.headers.get('SLIDER0', 0))
        slider1 = int(self.headers.get('SLIDER1', 0))
        slider2 = int(self.headers.get('SLIDER2', 0))
        message = self.headers.get('Msg', "")

        # Prepare data dictionary for microcontroller communication
        data = {
            "LCD": lcd,
            "SW0": sw0,
            "SW1": sw1,
